...
import datetime  # برای استفاده در ساختارهای زمانی
...
class ExplanationModal(discord.ui.Modal):
    def __init__(self, applicant: discord.User, original_embed: discord.Embed, reviewer: discord.User, action: str, original_message: discord.Message):
        ...
        :param original_embed: ایمبد فرم اولیه درخواست
        :param reviewer: کاربری که درخواست را بررسی می‌کند (Accept/Deny)
        :param action: "accept" یا "deny"
        :param original_message: پیام اصلی که دکمه‌های Accept/Deny روی آن قرار دارند
        """
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # تنظیم عنوان و رنگ نهایی بر اساس نوع اقدام
        if self.action == "accept":
            embed_title = "مجوز گواهینامه پذیرفته شده"
            color = discord.Color.green()
            target_channel_id = ACCEPT_CHANNEL_ID
        else:
            embed_title = "مجوز گواهینامه رد شده"
...
# ---------------------- Persistent View دکمه‌های Accept و Deny ----------------------
class ApprovalView(discord.ui.View):
    def __init__(self):
        # timeout=None به معنی عدم انقضا است
...
@bot.event
async def on_ready():
    # ثبت ویوی persistent برای دکمه‌های Accept و Deny
    bot.add_view(ApprovalView())
    await bot.tree.sync()  # همگام‌سازی دستورات اسلش
    print(f"Logged in as {bot.user}")

# ---------------------- دستور اسلش /govahiname ----------------------
@bot.tree.command(name="govahiname", description="شروع فرآیند درخواست")
async def govahiname(interaction: discord.Interaction):
    ...
    questions = [
        ...
        {"question": "آیا قبلاً گواهینامه‌ای داشته‌اید؟ بلی/خیر", "key": "had_driver_license"},
        {"question": "در صورت داشتن گواهینامه قبلی، علت باطل شدن گواهینامه قبلی ؟", "key": "driver_license_revoked_reason"},
    ...
    await dm_channel.send("شروع فرم درخواست دریافت گواهینامه, لطفاً به ترتیب به سوالات پاسخ دهید")
    ...
    embed = discord.Embed(title="فرم درخواست", color=discord.Color.yellow())
    ...
    embed.add_field(name="آیا قبلاً گواهینامه‌ای داشته‌اید؟ بلی/خیر", value=answers.get("had_driver_license", "ندارد"), inline=False)
    embed.add_field(name="در صورت داشتن گواهینامه قبلی، علت باطل شدن گواهینامه قبلی ؟", value=answers.get("driver_license_revoked_reason", "ندارد"), inline=False)
